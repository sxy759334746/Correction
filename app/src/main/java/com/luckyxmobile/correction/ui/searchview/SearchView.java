package com.luckyxmobile.correction.ui.searchview;
...
import com.luckyxmobile.correction.adapter.SearchAdapter;
...
public class SearchView extends LinearLayout {
    ...
    private int SEARCH_SELECTION;//TabLayout：0，1,2分别表示第一，二，三，个Item
    ...
    private void initView(){
        ...
        String COMPREHENSIVE = context.getString(R.string.comprehensive);
        String CORRECTIONBOOK = context.getString(R.string.correction_book);
        String REVIEWPAGE = context.getString(R.string.review_page);
        ...
        mTabLayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabLayout.Tab tab) {
                SEARCH_SELECTION = mTabLayout.getSelectedTabPosition();
                queryData(et_search.getText().toString().trim());
            }
    ...
    private void queryData(String tempName) {
        ...
        searchList = CorrectionLab.selectName(tempName,SEARCH_SELECTION);

        /**
         * 2. 创建adapter适配器对象 & 装入模糊搜索的结果
         * 3. 设置适配器
         * 历史记录是一个简单适配器
         * 搜索结果是自定义的适配器SearchAdapter
         */
        if (tempName.equals("")){
            //tempName为空时显示历史记录
            ...
            //tempName不为空时显示模糊搜索结果
            mTabLayout.setVisibility(View.VISIBLE);
            searchAdapter = new SearchAdapter(context,R.layout.search_item_listview,searchList);
            listView.setAdapter(searchAdapter);
            final List<Search> finalSearchList = searchList;
            listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    Search search = finalSearchList.get(position);
                    // 获取用户点击列表里的文字,并自动填充到搜索框内
                    et_search.setText(search.getName());

                    //判断搜索结果是错题本还是错题卷
                    if (search.getImageId() == R.drawable.ic_page){
                        //搜索结果是错题卷
                        Intent intent = new Intent(context, PaperDetailActivity.class);
                        intent.putExtra("reviewPaperId", search.getId()+"");
                        context.startActivity(intent);
                    }
                    if (search.getImageId() == R.drawable.correction_book){
                        //搜索结果是错题本
                        Intent intent = new Intent(context, BookDetailActivity.class);
                        intent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
                        intent.putExtra("book_id", search.getId());
                        context.startActivity(intent);
                    }

                    //对该搜索字段在数据库是否存在进行检查（查询）->> 关注1
                    boolean hasData = hasData(et_search.getText().toString().trim());
